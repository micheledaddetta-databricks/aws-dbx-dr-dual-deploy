locals {
  ...
  db_private_subnets_cidr = [cidrsubnet(var.cidr, 3, 0), cidrsubnet(var.cidr, 3, 1)]
  pl_private_subnets_cidr = [cidrsubnet(var.cidr, 3, 2)]
  nat_public_subnets_cidr = [cidrsubnet(var.cidr, 3, 1)]

  availability_zones = ["${var.location}a", "${var.location}b"]
  ...
  db_web_app                 = lookup(local.regional_config, var.location).db_web_app
  db_tunnel                  = lookup(local.regional_config, var.location).db_tunnel
  db_rds                     = lookup(local.regional_config, var.location).db_rds
  db_control_plane           = lookup(local.regional_config, var.location).db_control_plane
  vpc_endpoint_backend_rest  = lookup(local.regional_config, var.location).vpc_endpoint_backend_rest
  vpc_endpoint_backend_relay = lookup(local.regional_config, var.location).vpc_endpoint_backend_relay
}
